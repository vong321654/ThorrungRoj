...
export async function GET(request: Request) {
  ...
  const supabase = adminAuth instanceof Response ? customerAuth!.supabase : adminAuth.supabase;
...
export async function POST(request: Request) {
  ...
  if (kind === IMAGEKINDENUM.PRODUCT) {
    ...
    const uploadError = await uploadWebpImage(auth.supabase, PRODUCT_IMAGE_BUCKET, path, webpImage);
    if (uploadError) return Response.json(apiError("Failed to upload product image"), { status: 500 });
    const { data: urlData } = auth.supabase.storage.from(PRODUCT_IMAGE_BUCKET).getPublicUrl(path);
    const { data, error } = await auth.supabase
      .from("products")
    ...
    if (error) {
      await auth.supabase.storage.from(PRODUCT_IMAGE_BUCKET).remove([path]);
  ...
  const uploadError = await uploadWebpImage(supabase, RECEIPT_IMAGE_BUCKET, path, webpImage);
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) {
    await supabase.storage.from(RECEIPT_IMAGE_BUCKET).remove([path]);
...
export async function PATCH(request: Request) {
  ...
  const uploadError = await uploadWebpImage(auth.supabase, bucket, path, webpImage);
  if (uploadError) return Response.json(apiError("Failed to upload image"), { status: 500 });
  const imageValue = isProduct
    ? auth.supabase.storage.from(PRODUCT_IMAGE_BUCKET).getPublicUrl(path).data.publicUrl
    : path;
